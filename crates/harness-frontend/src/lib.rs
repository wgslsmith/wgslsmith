...
impl Printer {
    pub fn print_all_configs(&self, configs: Vec<Config>) -> eyre::Result<()> {
    ...
    pub fn print_default_configs(&self, configs: &[ConfigId]) -> eyre::Result<()> {
        let mut stdout = StandardStream::stdout(ColorChoice::Auto);
    ...
    pub fn print_execution_result(&self, result: ExecutionResult) -> eyre::Result<()> {
        let mut stdout = StandardStream::stdout(ColorChoice::Auto);
